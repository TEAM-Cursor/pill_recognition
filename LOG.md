...
## 2026-06-24
- **P3 알약 인식 — `apps/pill` 인식 파이프라인** (`feat/pill-identify`, 미커밋): 백엔드 4인 분담(P1 기반·P2 매칭·P3 인식·P4 상담) 중 P3. `POST /api/pill/identify`(multipart 1장, jpeg/png/webp ≤8MB, 사진 미저장) → Vision 속성 추출 → 매칭 → 후보 약. **도메인** `PillAttributes` + 식약처 낱알식별 enum(모양·색·분할선·제형). **포트** Vision/매칭(output)·Identify(input) Protocol, use_case `IdentifyPillUseCase`(2포트 생성자 주입). **Gemini Vision 어댑터**: 구조화 JSON 프롬프트(enum 강제)+파서, P1 공용 클라이언트를 `GeminiVisionClient` Protocol로 감싸 google-genai 직접 import 없이 CI 무사. **스텁**: Vision·매칭 fake로 DI 기본 주입 → P1/P2 없이 e2e 동작(완료 시 DI 한 곳 교체). 공용구역(`core`·`main.py`·`pyproject.toml`) 미변경 — main 라우터 등록은 P1 몫(주석 예시 그대로). 검증: ruff·mypy(94)·lint-imports(5 KEPT)·pytest(10) 그린.
